...
//Delete rows which are properly classified
$sql="delete from $training_table where lr_predict=is_malignant";
db_query($sql);
echo "Deleted properly classified rows\n";
$sql=<<<EOL
select * from knn(
{
"training_table":"$training_table",
...
"output_table":"nn_output",
"target_class":"is_malignant",
"features":["x1","x2","x3","x4","x5","x6","x7","x8","x9","nb_predict"]
})
EOL;
$result=db_query_tesla($sql);
while($row=db_fetch_array($result))
